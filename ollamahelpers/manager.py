...
def wait_for_server(host, port, wait, max_attempts):
    ...
    while not is_server_ready(host, port):
        ...
        if attempts > max_attempts:
            raise TimeoutError(f'no answer from server {server} after {attempts-1} attempts')
        print(f'Attempt {attempts:2d}/{max_attempts}: Waiting for ollama {server}...')
        time.sleep(wait)
...
def start_ollama(
    ...
    print(f'INFO   | checking if the {get_server(host, port)} is up...')
    wait_for_server(host, port, wait, attempts)
    print(f'SUCCESS| ollama server {get_server(host, port)} is up')
